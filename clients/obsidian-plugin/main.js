...
var zh = (window.localStorage.getItem("language") || "").startsWith("zh");
var T = zh ? {
...
var DdlRadarPlugin = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        new import_obsidian.Notice(T.refreshed);
      }
    });
    this.addCommand({
      id: "insert-table",
  ...
  async refreshData(force) {
    ...
    try {
      ...
      if (this.confs.length) new import_obsidian.Notice(T.offline);
    }
    this.afterDataChanged();
  }
  afterDataChanged() {
  ...
  updateStatusBar() {
    ...
    this.statusEl.setText(urgent ? T.dueSoon(urgent) : "\u{1F4E1}");
  ...
  maybeRemind() {
    ...
    for (const r of due) {
      new import_obsidian.Notice(`\u{1F4E1} ${r.c.name}: ${T.days(r.d)} (${r.abs ? T.abs + " " : ""}${fmtDate(r.iso)})`, 8e3);
    }
  }
  renderBoard(el, f) {
    el.empty();
    ...
    for (const ref of refs) {
      ...
      if (nd) {
        ...
        banner.createSpan({
          ...
          text: `${T.days(d)} ${nd.abs ? T.absFirst : T.bannerDdl} \xB7 ${fmtDate(nd.iso)}`
        });
...
var RadarSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName(zh ? "\u63D0\u9192\u63D0\u524D\u91CF\uFF08\u5929\uFF09" : "Reminder lead time (days)").setDesc(
      zh ? "\u542F\u52A8\u65F6\uFF0C\u6536\u85CF\u7684\u4F1A\u8BAE\u5728 N \u5929\u5185\u622A\u6B62\u4F1A\u5F39\u51FA\u63D0\u9192\uFF08\u6BCF\u5929\u6700\u591A\u4E00\u6B21\uFF09" : "On startup, show a notice for starred conferences due within N days (at most once a day)"
    ).addSlider(
    ...
    new import_obsidian.Setting(containerEl).setName(zh ? "\u81EA\u52A8\u5237\u65B0\u95F4\u9694\uFF08\u5C0F\u65F6\uFF09" : "Auto-refresh interval (hours)").setDesc(zh ? "\u4ECE\u516C\u5F00\u6570\u636E\u96C6\u62C9\u53D6\u6700\u65B0\u622A\u7A3F\u6570\u636E\u7684\u9891\u7387" : "How often to re-fetch the public dataset").addSlider(
    ...
    new import_obsidian.Setting(containerEl).setName(zh ? "\u9ED8\u8BA4\u65F6\u95F4\u7A97\u53E3\uFF08\u5929\uFF09" : "Default window (days)").setDesc(
      zh ? "\u4EE3\u7801\u5757\u4E0E\u63D2\u5165\u8868\u683C\u9ED8\u8BA4\u53EA\u663E\u793A N \u5929\u5185\u7684\u622A\u7A3F" : "Code blocks and inserted tables show deadlines within N days by default"
    ).addSlider(
    ...
    new import_obsidian.Setting(containerEl).setName(zh ? "\u6570\u636E\u6765\u6E90" : "Data source").setDesc(DATA_URL).addButton(
      (b) => b.setButtonText(zh ? "\u7ACB\u5373\u5237\u65B0" : "Refresh now").onClick(async () => {
        await this.plugin.refreshData(true);
        new import_obsidian.Notice(T.refreshed);
      })
    );
